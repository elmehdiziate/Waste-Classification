...
class WaRPPreprocessor:
    ...
    def _apply_extra_augs(self, tfm_list):
        if self.extra_rotation:
            tfm_list.insert(2, T.RandomRotation(20))

        if self.extra_color:
            tfm_list.insert(3, T.ColorJitter(brightness=0.2, contrast=0.2))

        if self.extra_blur:
            tfm_list.insert(-2, T.RandomApply([T.GaussianBlur(3)], p=0.2))
    ...
    def _get_pretrained_cnn_transforms(self, crop_scale_min: float = 0.6) -> T.Compose:
        ...
        # Apply Phase 3 optional augmentations
        tfm = self._apply_extra_augs(tfm)
